...
**How to apply:** For any non-trivial work, create a feature branch (e.g., `fix/lint-cleanup`, `feat/integration-ui`), commit there, push with `-u`, then open a PR via `gh pr create`. Group related changes into logical PRs. Wait for user to confirm merge unless told otherwise.
