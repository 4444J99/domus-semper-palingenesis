...
**Design phase: COMPLETE.** SOP written, env vars integrated into chezmoi pipeline, all 9 domains confirmed available.
**Execution phase: IN PROGRESS.** Session interrupted mid-audit. Uncommitted changes in two repos. IRF not yet updated. Memory persistence VACUUM discovered and unresolved.

### Completed (prior session)
- [x] `~/.config/chezmoi/chezmoi.toml` — 8 `domain_*` data fields added
- [x] `dot_zshenv.tmpl` — 8 identity-layer DOMAIN_* vars (template, renders to `~/.zshenv`)
- [x] `dot_config/zsh/15-env.zsh` — 13 derived-layer DOMAIN_* vars (organ subdomains, API, status, shortener, portfolio, resume)
- [x] `meta-organvm/praxis-perpetua/standards/SOP--domain-architecture-and-dns.md` — Full SOP, SPEC-023 format
- [x] `domus/.claude/plans/2026-04-16-domain-architecture-concentric-circles.md` — Plan archived
- [x] Memory entry `project_domain_architecture.md` + MEMORY.md pointer

### Pending — must complete before session close
- [ ] **CRITICAL: Fix memory VACUUM** — `.chezmoiignore` line 92 excludes `.claude/projects/*/memory/`. Remove this exclusion, `chezmoi add` the memory files, then `chezmoi apply` (auto-commits+pushes). Achieves `[(local):(remote)={1:1}]`.
- [ ] **Commit domus repo** — stage+commit: `dot_zshenv.tmpl`, `dot_config/zsh/15-env.zsh`, `.claude/plans/2026-04-16-domain-architecture-concentric-circles.md`. Then `chezmoi apply` to trigger auto-commit+push.
- [ ] **Commit praxis-perpetua SOP** — stage+commit `standards/SOP--domain-architecture-and-dns.md` in the `meta-organvm/praxis-perpetua` repo.
- [ ] **Update IRF** — add to `meta-organvm/organvm-corpvs-testamentvm/INST-INDEX-RERUM-FACIENDARUM.md`:
  - IRF-DOM-031: Domain architecture + 21 env vars implemented (DONE-338)
  - IRF-DOM-032: Memory persistence VACUUM (`.chezmoiignore` excludes `memory/`) — open item
  - IRF-PRT-026: Portfolio domain migration — Phase 3 of SOP (base change `/portfolio`→`/`, Cloudflare Pages, high-risk)
- [ ] **10-index propagation** — GitHub issue for domain registration task (triple reference rule), testament event, concordance entries
- [ ] **Copy plan to project** — copy this plan file to `domus/.claude/plans/2026-04-16-domain-architecture-concentric-circles.md` (plan file discipline)
