...
Make npm the *declared* package manager (not just the de-facto one) and remove the stale pnpm artifact. This eliminates the VS Code warning at its root rather than silencing it via the `npm.packageManager` setting.
